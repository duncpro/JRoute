...
class RouteTreeNode<E> {
     ...
     */
    private RouteTreeNode(@Nullable RouteTreeNode<E> parent, @Nullable RouteElement associatedRouteElement) {
        this.parent = parent;
        this.associatedRouteElement = associatedRouteElement;
    }

    public static <E> RouteTreeNode<E> newTree() { return new RouteTreeNode<>(null, null); }
    ...
    private boolean hasGreedyChild() {
        return children.size() == 1 && children.get(0).associatedRouteElement instanceof WildcardRouteElement;
    }
    ...
    private void addChildRoute(RouteTreeNode<E> childNode) throws RouteConflictException {
        ...
        if (childNode.associatedRouteElement instanceof WildcardRouteElement && !children.isEmpty()) {
    ...
    Optional<RouteTreeNode<E>> matchChildRoute(String pathElement) {
        if (hasGreedyChild()) {
            ...
            return children.stream()
                    .filter(child -> JRouteUtilities.accepts(pathElement, child.associatedRouteElement))
    ...
    RouteTreeNode<E> getOrCreateChildRoute(RouteElement trailingRouteElement) {
        return children.stream()
                .filter(child -> Objects.equals(child.associatedRouteElement, trailingRouteElement))
                .reduce(($, $$) -> { throw new AssertionError(); })
                .orElseGet(() -> {
                    final var newChild = new RouteTreeNode<E>(this, trailingRouteElement);
    ...
    private boolean isRoot() { return this.associatedRouteElement == null; }
    ...
    private static Route traverseToRoot(RouteTreeNode<?> node) {
        ...
        while (!node.isRoot()) {
            route = route.withLeadingElement(node.associatedRouteElement);
            node = node.parent;
        }
